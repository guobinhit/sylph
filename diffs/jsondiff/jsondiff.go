...
// DefaultConsoleOptions use ANSI foreground color escape sequences to highlight changes.
...
func (ctx *context) result(d DiffType) {
	if d == NoMatch {
		ctx.diffType = NoMatch
	} else if d == SupersetMatch && ctx.diffType != NoMatch {
		ctx.diffType = SupersetMatch
	} else if ctx.diffType != NoMatch && ctx.diffType != SupersetMatch {
		ctx.diffType = FullMatch
	}
}
...
func Compare(a, b []byte, opts ...Options) (DiffType, string) {
	...
	var buf bytes.Buffer
	ctx := context{opts: &opts[0]}
	buf.WriteString(ctx.printDiff(av, bv))
	return ctx.diffType, buf.String()
}
